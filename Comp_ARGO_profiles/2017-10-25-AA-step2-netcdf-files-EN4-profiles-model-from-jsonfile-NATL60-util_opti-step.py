...
def open_natl60(jsonfile,infos,nb_profilesEN4):
        print 'Opening NATL60'
	dirn="/scratch/cnt0024/hmg2840/albert7a/NATL60/NATL60-CJM165-S/TS/"
	tsec=np.zeros(nb_profilesEN4)
        list_profiles = infos.keys()
	...
	idmin=np.where(tsec == np.min(tsec))
	idmax=np.where(tsec == np.max(tsec))
        datemin=str(infos[list_profiles[idmin[0]]]['date'])
        datemax=str(infos[list_profiles[idmax[0]]]['date'])
       	yearmin=datemin[0:4]
	yearmax=datemax[0:4]
	...
	if int(monthmin) == 1:
		if int(monthmax) > 10:
			...
			ymin=int(yearmin)-1
			ymax=int(yearmax)+1
			list_files=dirn+"NATL60-CJM165_y"+str(ymin)+"m12d??.1d_TS.nc"
			if 'tfiles' in locals():
				tfiles=tfiles+glob.glob(list_files)
			else:
				tfiles=glob.glob(list_files)
			for m in np.arange(1,13):
				if m < 10:
					list_files=dirn+"NATL60-CJM165_y"+year+"m0"+str(m)+"d??.1d_TS.nc"
				else:
					list_files=dirn+"NATL60-CJM165_y"+year+"m"+str(m)+"d??.1d_TS.nc"
				if 'list_files' in locals():
					tfiles=tfiles+glob.glob(list_files)
				else:
					tfiles=glob.glob(list_files)
			list_files=dirn+"NATL60-CJM165_y"+str(ymax)+"m01d??.1d_TS.nc"
			if 'tfiles' in locals():
				tfiles=tfiles+glob.glob(list_files)
			else:
				tfiles=glob.glob(list_files)
			...
			if 'tfiles' in locals():
				...
				tfiles=glob.glob(list_files)
                        for m in np.arange(1,int(monthmax)+2):
                                if m < 10:
                                        list_files=dirn+"NATL60-CJM165_y"+year+"m0"+str(m)+"d??.1d_TS.nc"
                                else:
                                        list_files=dirn+"NATL60-CJM165_y"+year+"m"+str(m)+"d??.1d_TS.nc"
		...
		if int(monthmax) > 10:
			year=yearmin
                        ymax=int(yearmax)+1
                        for m in np.arange(int(monthmin)-1,13):
                                if m < 10:
                                        list_files=dirn+"NATL60-CJM165_y"+year+"m0"+str(m)+"d??.1d_TS.nc"
                                else:
                                        list_files=dirn+"NATL60-CJM165_y"+year+"m"+str(m)+"d??.1d_TS.nc"
			...
			if yearmin == yearmax:
				year=yearmin
				for m in np.arange(int(monthmin)-1,int(monthmax)+2):
	                                if m < 10:
        	                                list_files=dirn+"NATL60-CJM165_y"+year+"m0"+str(m)+"d??.1d_TS.nc"
                	                else:
                        	                list_files=dirn+"NATL60-CJM165_y"+year+"m"+str(m)+"d??.1d_TS.nc"
				...
				for year in np.arange(int(yearmin)-1,int(yearmax)+2):
					for month in np.arange(int(monthmin)-1,int(monthmax)+2):
	                                        if m < 10:
        	                                        list_files=dirn+"NATL60-CJM165_y"+year+"m0"+str(m)+"d??.1d_TS.nc"
                	                        else:
	                                                list_files=dirn+"NATL60-CJM165_y"+year+"m"+str(m)+"d??.1d_TS.nc"
...
def model_mean_percent_profile(fileEN4,ref_prof,dsN,latN,lonN,timN):
    ...
    def profile_mean_percent(k):
        ...
        lon_stacked = lonB[0::3].stack(profile=('x', 'y'))
        lat_stacked = latB[0::3].stack(profile=('x', 'y'))
        ...
        model_temperature_stacked = model_temperatureB[:,0::3].stack(profile=('x', 'y'))
        model_salinity_stacked = model_salinityB[:,0::3].stack(profile=('x', 'y'))
...
def main():
    ...
    dsN,latN,lonN,timN = open_natl60(jsonfile,infos,nb_profilesEN4)
